<?php 
	...
	$query = "SELECT topics.title, teams.name, teams.groups, teamtopics.id
			  FROM teamtopics
			  INNER JOIN teams ON teamtopics.teamid = teams.id 
			  INNER JOIN topics ON teamtopics.topicid = topics.id
			  ORDER BY teams.groups, teams.name ASC LIMIT $start_from, $limit";
...
<div class="content" style="width: 650px;">
	...
	<div class="topnav" id="myTopnav">
	  <a class="active" href="settingsAdmin.php">1. Timovi i teme</a>
	...
	<!-- ISPIS TIMOVA I TEMA -->
	<br>
	<p><h2 align="center">1. Timovi i teme</h2></p><br>
	<p align="center"> Odaberite temu koju želite osloboditi za izbor drugim timovima. </p>

	<form name="teamtopic" action="settingsAdmin.php?page=<?php echo $page;?>" method="post" style="display: table; background: transparent; border-color: transparent; width: auto;">
		...
		<table>
			<thead>
				<tr>
					...
					<th>Grupa</th>
					<th>Tema</th>
					<th style="text-align: center;">Odaberi <br><input type="checkbox" id="cbgroup1_master" onchange="togglecheckboxes(this, 'topics[]')"> </th>
				</tr>
			</thead>
			<tbody>
				<?php while ($row = mysqli_fetch_array($result)) { ?>
				    <tr>
				    	...
				    	<td style="text-align: center;"> <input type="checkbox" name="topics[]" value="<?php echo $row["id"]; ?>" /> </td>
				    </tr>
				<?php } ?>
			</tbody> 
		</table>
	...
	<div class="input-group">
	<button type="submit" class="delbtn" name="del_teamtopic" onclick="return confirm('Sigurno želite obrisati odabrano?')">Oslobodi odabranu temu</button>
	</div>

	</form>

	<?php  
		$query = "SELECT COUNT(id) FROM teamtopics";  
		...
		for ($i=1; $i<=$total_pages; $i++) {  
		             $pagLink .= "<li><a href='settingsAdmin.php?page=".$i."'>".$i."</a></li>";  
		};  
		echo $pagLink . "</ul></nav>";  
	?>
...
<script type="text/javascript">
	...
	$('.pagination').pagination({
	        ...
	        hrefTextPrefix : 'settingsAdmin.php?page='
	    });
	    });
</script>
</body>
